...
>>>>> 重要说明：
	...
	*	        如果使用内存映射模式，则读取性能只与QSPI的驱动时钟以及是否开启Cache有关
        *
	...
	*               W25Q64JV 所允许的最高驱动频率为133MHz，750的QSPI最高驱动频率也是133MHz ，但是对于HAL库函数直接读取而言，驱动时钟超过15M已经不会有性能提升
	*	  对于内存映射模式直接读取而言，驱动时钟超过127.5M已经不会有性能提升，因为QSPI内核时钟最高限定为250M，所以建议实际QSPI驱动时钟不要超过125M，
	*	        具体的时钟配置请参考 sysclk_config 函数
...
/*在某些场合，例如用作下载算法时，需要手动清零句柄等参数，否则会工作不正常*/	
	...
	/*本例程选择 HCLK 作为QSPI的内核时钟，速度为240M，再经过2分频得到120M驱动时钟，
	  关于 QSPI内核时钟 的设置，请参考 main.c文件里的 sysclk_config 函数*/
...
int8_t QSPI_W25Qxx_MMMode(void)
{
